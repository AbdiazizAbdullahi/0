...
export default function ClientDetail() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setError(null)
      try {
        const response = await window.electronAPI.mainOperation("getClientDetails", id)
        if (response.success) {
  ...
  const exportToCSV = () => {
    ...
    link.href = url
    link.setAttribute('download', `client_${id}_ledger.csv`)
  ...
  const exportToPDF = async () => {
    try {
      const response = await window.electronAPI.mainOperation("clientPDF", {
        data: filteredData,
        client: data.info.name,
        totals: data.metrics
      })
      ...
      <Card className="w-full">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">View all transactions for this client</p>
          </div>
