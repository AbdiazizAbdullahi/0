          ...
          <tbody>
            {paginatedSuppliers.map((supplier) => (
              <tr key={supplier._id} className="border-b">
                ...
                <td className={`p-4 ${supplier.balance >= 0 ? 'text-green-600' : 'text-red-600'}`}>{supplier.balance >= 0 ? `${supplier.currency} ${formatCurrency(supplier.balance)}` : `${supplier.currency} [-${formatCurrency(Math.abs(supplier.balance))}]`}</td>
